...
//                     router_opcodes.op_i64_const32(state_value);
...
//                         router_opcodes.op_i64_const32(router_offset + router_opcodes.len() + 2 +
// 1);
//
//                         router_opcodes.op_call_internal(func_index);
//                         if let Some(output_code) = &self.config.output_code {
...
//
//                 router_opcodes.extend(&check_instr);
//                 router_opcodes.op_i64_const32(INIT_PRELUDE_VALUE);
//                 // if states are not equal then skip this call
//                 router_opcodes.op_i32_eq();
...
//         // inject main function call with return
//         self.code_section.op_i64_const32(return_offset);
...
//         // inject main function call with return
//         self.code_section.op_i64_const32(return_offset);
//         self.code_section.op_call_internal(func_index);
//         // self.code_section.op_br_indirect(0);
//         self.code_section.op_unreachable();
//         Ok(())
//     }
...
//                         DropKeep::new(drop_keep.drop() as usize + 1, drop_keep.keep() as usize +
// 1) .map_err(|_| CompilerError::DropKeepOutOfBounds)?, )?; } }
//
//             br_table_status
//                 .injection_instructions
...
//                 self.code_section.op_i64_const32(888);
//                 self.code_section.op_drop();
//
//                 let drop_keep = Self::extract_drop_keep(instr_ptr);
...
//
//                 let idx = self.get_or_insert_check_idx(func_type.clone());
//                 self.code_section.op_i64_const32(idx);
//
//                 self.swap_stack_parameters(1);
//                 // self.code_section.op_br_indirect(0);
//             }
...
//                 self.code_section.op_i64_const32(target);
//                 let fn_index = func_idx.into_usize() as u32 + self.module.imports.len_funcs as
// u32;                 let call_func_type = self.module.funcs[fn_index as usize];
//                 let func_type = self.engine.resolve_func_type(&call_func_type, Clone::clone);
//                 let type_index = self.get_or_insert_check_idx(func_type.clone());
//                 if self.config.type_check {
