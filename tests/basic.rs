...
fn test_i64_mul() {
    let wasm_binary = wat::parse_str(
        r#"
(module
  (func (export "i64mul") (param i64 i64) (result i64)
    local.get 0
    local.get 1
    i64.mul
  )
...
"#,
    ...
    let config = CompilationConfig::default()
        .with_entrypoint_name("i64mul".into())
        .with_allow_malformed_entrypoint_func_type(true);
    let (rwasm_module, _) = RwasmModule::compile(config, &wasm_binary).unwrap();
    for instr in rwasm_module.code_section.iter() {
        println!("{}", instr);
    }
    let mut store = RwasmStore::<()>::default();
    let mut engine = ExecutionEngine::new();
    ...
    assert!(matches!(execution_result, Ok(_)));

    assert_eq!(result[0].i64().unwrap(), a * b);
}
