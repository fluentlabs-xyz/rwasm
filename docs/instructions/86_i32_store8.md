...
The `i32_store8` instruction **stores the lower 8 bits of a 32-bit integer to memory** at a specified address with an optional offset. It pops a value and a memory address from the stack, computes the effective address, and stores the lower 8 bits of the value at that location.
...
5. **Extracts** the lower 8 bits of the value.
6. **Stores** the 8-bit value to memory at the computed address.
...
- Stores exactly 1 byte (8 bits) to memory.
- Only the **lower 8 bits** of the 32-bit value are stored; the upper 24 bits are ignored.
- Equivalent to storing a `uint8_t` to memory at `address + offset`.
