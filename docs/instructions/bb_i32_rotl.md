...
The `i32_rotl` instruction **performs left rotation operation on a 32-bit integer**. It pops two 32-bit integers from the stack (value and rotation amount), rotates the value left by the specified number of bits, and pushes the result back onto the stack.
...
4. **Performs** left rotation operation on the value.
...
Where `result` is `value` rotated left by `rotation_amount & 0x1F` bits.
...
- Left rotation by n bits is equivalent to: `(value << n) | (value >> (32 - n))`.
- Rotation is **reversible** - `rotl(rotl(x, n), 32-n) = x`.
