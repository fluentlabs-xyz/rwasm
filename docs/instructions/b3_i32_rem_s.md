...
The `i32_rem_s` instruction **computes the signed remainder of dividing two 32-bit integers**. It pops two 32-bit integers from the stack, interprets them as signed integers, computes the remainder, and pushes the result back onto the stack.
...
4. **Performs** signed remainder operation of the two values.
...
- Remainder is computed using **signed interpretation** of the 32-bit integers.
- **Can trap** on division by zero, but not on overflow (unlike `i32_div_s`).
- The result has the same sign as the dividend (C-style remainder behavior).
- For `INT32_MIN % -1`, the result is 0 (no trap, unlike division).
