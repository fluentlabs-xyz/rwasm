...
use crate::{ExternRef, FuncIdx, FuncRef, UntypedValue, Value, F32, F64};
use alloc::boxed::Box;
use core::fmt;
...
/// A [`InitExpr`] evaluation context.
///
/// Required for evaluating a [`InitExpr`].
...
/// An input parameter to a [`InitExpr`] operator.
#[derive(Debug)]
pub enum Op {
    /// A constant value.
    Const(ConstOp),
...
#[derive(Debug, Clone)]
pub struct InitExpr {
    /// The root operator of the [`InitExpr`].
    pub(crate) op: Op,
}

impl Eval for InitExpr {
    fn eval(&self, ctx: &dyn EvalContext) -> Option<UntypedValue> {
        self.op.eval(ctx)
    }
}
...
impl InitExpr {
    ...
    /// Creates a new [`InitExpr`] from the given Wasm [`InitExpr`].
    ///
    /// # Note
    ///
    /// The constructor assumes that Wasm validation already succeeded
    /// on the input Wasm [`InitExpr`].
    ...
    /// Evaluates the [`InitExpr`] in a constant evaluation context.
    ///
    /// # Note
    ///
    ...
    /// Evaluates the [`InitExpr`] given a context for globals and functions.
    ///
    ...
    /// # Note
    ///
    /// This is useful for evaluation of [`InitExpr`] during bytecode execution.
