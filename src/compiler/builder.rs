use crate::{
    compiler::{error::CompilerError, init_expr::InitExpr, CompilerConfig},
    CompiledFunc,
    FuncIdx,
    GlobalIdx,
    ImportName,
...
pub struct ModuleBuilder<'a> {
    ...
    pub exports: BTreeMap<Box<str>, (ExternalKind, u32)>,
    pub start: Option<u32>,
    pub compiled_funcs: Vec<u32>,
    ...
    pub fn get_compiled_func(&self, func_idx: FuncIdx) -> Option<CompiledFunc> {
        let index = if let Some(rwasm_config) = self.engine().config().get_rwasm_config() {
            if rwasm_config.wrap_import_functions {
                ...
                let index = (func_idx as usize).checked_sub(self.res.imports.len_funcs())?;
            ...
            (func_idx as usize).checked_sub(self.res.imports.len_funcs())?
        };
...
impl<'engine> ModuleBuilder<'engine> {
    ...
    pub fn new(config: &'engine CompilerConfig) -> Self {
        Self {
            config,
            func_types: Vec::new(),
    ...
    pub fn push_func_types<T>(&mut self, func_types: T) -> Result<(), CompilerError>
    where
        T: IntoIterator<Item = Result<FuncType, CompilerError>>,
    {
        assert!(
    ...
    pub fn push_func_type(&mut self, func_type: FuncType) -> Result<(), CompilerError> {
    ...
    ///
    /// If this function has already been called on the same [`ModuleBuilder`].
    pub fn push_imports<'a, T>(&mut self, imports: T) -> Result<(), CompilerError>
    where
        T: IntoIterator<Item = Result<Import<'a>, CompilerError>>,
    {
        for import in imports {
            ...
            match import.ty {
                TypeRef::Func(func_type_idx) => {
                    ...
                    let func_type = self.func_types[func_type_idx as usize].clone();
                ...
                TypeRef::Tag(_) => unreachable!("rwasm: exceptions are not allowed"),
            }
        }
        Ok(())
    }
    ...
    ///
    /// If this function has already been called on the same [`ModuleBuilder`].
    pub fn push_funcs<T>(&mut self, funcs: T) -> Result<(), CompilerError>
    where
        T: IntoIterator<Item = Result<u32, CompilerError>>,
    {
        assert_eq!(
            self.funcs.len(),
        ...
        for func in funcs {
            ...
            self.compiled_funcs.push(func_type_idx);
        }
        Ok(())
    }
    ...
    ///
    /// If this function has already been called on the same [`ModuleBuilder`].
    pub fn push_tables<T>(&mut self, tables: T) -> Result<(), CompilerError>
    where
        T: IntoIterator<Item = Result<TableType, CompilerError>>,
    {
        assert_eq!(
            self.tables.len(),
    ...
    ///
    /// If this function has already been called on the same [`ModuleBuilder`].
    pub fn push_memories<T>(&mut self, memories: T) -> Result<(), CompilerError>
    where
        T: IntoIterator<Item = Result<MemoryType, CompilerError>>,
    {
        assert_eq!(
            self.memories.len(),
    ...
    ///
    /// If this function has already been called on the same [`ModuleBuilder`].
    pub fn push_globals<'a, T>(&mut self, globals: T) -> Result<(), CompilerError>
    where
        T: IntoIterator<Item = Result<Global<'a>, CompilerError>>,
    {
        assert_eq!(
            self.globals.len(),
        ...
        for global in globals {
            ...
            self.globals_init.push(InitExpr::new(init_expr));
        }
        Ok(())
    }

    pub fn rewrite_memory(&mut self) -> Result<(), CompilerError> {
        // rewrite a memory section
    ...
    pub fn rewrite_tables(&mut self) -> Result<(), CompilerError> {
        // rewrite tables
    ...
    ///
    /// If this function has already been called on the same [`ModuleBuilder`].
    pub fn push_exports<'a, T>(&mut self, exports: T) -> Result<(), CompilerError>
    where
        T: IntoIterator<Item = Result<Export<'a>, CompilerError>>,
    {
        assert!(
        ...
        //             Ok((field, export.kind, export.index))
        self.exports = exports.into_iter().collect::<Result<BTreeMap<_, _>, _>>()?;
        Ok(())
    }

    pub fn rewrite_exports<I>(&mut self, name: Box<str>, index: I) -> Result<(), CompilerError>
    where
        I: Into<ExternIdx>,
    {
        self.exports.clear();
    ...
    ///
    /// If this function has already been called on the same [`ModuleBuilder`].
    pub fn push_element_segments<'a, T>(&mut self, elements: T) -> Result<(), CompilerError>
    where
        T: IntoIterator<Item = Result<Element<'a>, CompilerError>>,
    {
        assert!(
    ...
    ///
    /// If this function has already been called on the same [`ModuleBuilder`].
    pub fn push_data_segments<'a, T>(&mut self, data: T) -> Result<(), CompilerError>
    where
        T: IntoIterator<Item = Result<Data<'a>, CompilerError>>,
    {
        assert!(
