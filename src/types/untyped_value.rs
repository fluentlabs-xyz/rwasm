...
impl Encode for UntypedValue {
    fn encode<E: Encoder>(&self, encoder: &mut E) -> Result<(), EncodeError> {
        (self.bits as u64).encode(encoder)
    }
}

impl<Context> Decode<Context> for UntypedValue {
    fn decode<D: Decoder<Context = Context>>(decoder: &mut D) -> Result<Self, DecodeError> {
        let value = u64::decode(decoder)?;
        Ok(Self { bits: value as u32 })
    }
}
...
impl_from_untyped_for_int!(i8, i16, i32, u8, u16, u32);
...
impl_from_untyped_for_float!(f32, F32);
