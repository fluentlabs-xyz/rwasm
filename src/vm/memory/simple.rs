...
impl IGlobalMemory for SimpleGlobalMemory {
    ...
    fn reset(&mut self) {
        self.shared_memory.fill(0);
        let initial_len = self
            .initial_pages
            .to_bytes()
            .expect("rwasm: not supported target pointer width");
        unsafe {
            self.shared_memory.set_len(initial_len);
        }
        self.current_pages = self.initial_pages;
    }

    fn resize_for(&mut self, initial_pages: Pages) {
        let bytes_required = initial_pages.to_bytes().unwrap();
        if self.shared_memory.len() < bytes_required {
            self.shared_memory.resize(bytes_required, 0);
        }
    }
}
